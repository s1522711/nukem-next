...
import { addItem } from '@/app/actions/admin'
...
export function AdminItemsManager({ items }: { items: Item[] }) {
  ...
  return (
    <>
      ...
      {/* Add Asset Modal */}
      <div className={`fixed inset-0 z-50 flex items-center justify-center bg-obsidian/90 backdrop-blur-sm p-4 lg:p-8 transition-all duration-300 ${isModalOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className={`w-full max-w-5xl h-full max-h-[90vh] relative flex flex-col transform transition-all duration-300 ${isModalOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
            ...
            <div className="relative z-10 w-full flex-1 flex flex-col">
              ...
              <form action={handleSubmit} className="flex flex-col flex-1">
                  ...
                  <div className="flex flex-col h-full space-y-4">
                    ...
                    <div className="bg-obsidian-light/30 p-6 border border-obsidian-border flex-1 flex flex-col space-y-6 justify-center">
                      ...
                      <div className="flex-1 flex flex-col">
                        ...
                        <div className={`relative border border-dashed border-cyan-glow/30 p-4 bg-obsidian flex-1 flex items-center justify-center hover:bg-cyan-glow/5 transition-colors cursor-pointer min-h-[80px] mb-4 ${selectedFileName ? 'bg-cyan-glow/10 border-cyan-glow' : ''}`}>
                          <input name="imageFile" type="file" accept="image/*" onChange={(e) => setSelectedFileName(e.target.files?.[0]?.name || null)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
                          <div className="text-cyan-glow font-mono text-xs uppercase tracking-widest pointer-events-none text-center truncate px-4">
                            {selectedFileName ? `[ ${selectedFileName} ]` : '[ CLICK_OR_DRAG_FILE_HERE ]'}
                          </div>
                        </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 mt-8 pt-6 border-t border-cyan-glow/20 w-full">
                  ...
                  <SubmitButton loadingText="PROCESSING_UPLINK..." className="flex items-center justify-center text-center w-full sm:w-2/3 tactical-border-sm bg-cyan-glow/10 border border-cyan-glow text-cyan-glow font-bold uppercase tracking-[0.3em] hover:bg-cyan-glow hover:text-obsidian hover:box-shadow-cyan transition-all duration-300 py-4 px-8 text-xs order-1 sm:order-2">
                    AUTHORIZE_INSERTION
                  </SubmitButton>
                </div>
              </form>
